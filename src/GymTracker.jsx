          ...
          <div>
            ...
            <div style={{marginBottom:14}}>
              ...
              <div style={{display:"flex",flexDirection:"column",gap:6}}>
                ...
                {editing&&(!showAddProg?<button onClick={()=>setShowAddProg(true)} style={{padding:12,borderRadius:10,border:`2px dashed ${t.border}`,background:"transparent",color:t.textFaint,fontWeight:600,fontSize:12,cursor:"pointer"}}>+ New Program</button>:(
                  <div style={{display:"flex",gap:6}}><input value={newProgName} onChange={e=>setNewProgName(e.target.value)} onKeyDown={e=>e.key==="Enter"&&addProg()} placeholder="Program name..." autoFocus style={{flex:1,padding:"10px 12px",borderRadius:10,border:`1px solid ${t.border}`,background:t.inputBg,color:t.text,fontSize:14,outline:"none"}}/><button onClick={addProg} style={{padding:"10px 16px",borderRadius:10,border:"none",background:t.accent,color:"#fff",fontWeight:700,fontSize:12,cursor:"pointer"}}>Add</button></div>
                ))}
              </div>
            </div>
