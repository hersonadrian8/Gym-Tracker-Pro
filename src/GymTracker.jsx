...
export default function GymTracker({ user, signOut }) {
  ...
  const [historySwapIdx, setHistorySwapIdx] = useState(null);
  const [historySwapSearch, setHistorySwapSearch] = useState("");
  ...
  const saveEditHistory = () => { if (!editingHistory) return; setHistory(prev => { const updated = [...prev]; editingHistory.forEach(edited => { const idx = updated.findIndex(h => h.exercise === edited._origExercise && h.isoDate === edited.isoDate && h.split === edited.split); if (idx !== -1) updated[idx] = {...updated[idx], exercise: edited.exercise, weight: edited.weight, reps: edited.reps, sets: edited.sets, setDetails: edited.setDetails}; }); return updated; }); setEditingHistory(null); };
  const swapHistoryExercise = (idx, newEx) => { setEditingHistory(prev => prev.map((h, i) => i === idx ? {...h, exercise: newEx.name} : h)); setHistorySwapIdx(null); setHistorySwapSearch(""); };
  ...
  const deleteCardioEntry = (entry) => { setCardioHistory(prev => prev.filter(c => !(c.type === entry.type && c.isoDate === entry.isoDate && c.distance === entry.distance && c.time === entry.time))); };
  const renameExercise = (oldName, newName) => { if (!newName.trim() || newName.trim() === oldName) { setRenamingEx(null); setRenameVal(""); return; } const n = newName.trim(); setCustomExercises(prev => { const next = {...prev}; const data = next[oldName] || fullExDB[oldName] || { muscle: "Chest", rest: 90 }; delete next[oldName]; next[n] = {...data}; return next; }); if (customRestTimes[oldName]) setCustomRestTimes(prev => { const next = {...prev}; next[n] = next[oldName]; delete next[oldName]; return next; }); setFavoriteExercises(prev => { if (!prev.has(oldName)) return prev; const next = new Set(prev); next.delete(oldName); next.add(n); return next; }); setHistory(prev => prev.map(h => h.exercise === oldName ? {...h, exercise: n} : h)); setPrograms(prev => prev.map(p => ({...p, splits: p.splits.map(s => ({...s, exercises: s.exercises.map(e => e.name === oldName ? {...e, name: n} : e)}))}))); setRenamingEx(null); setRenameVal(""); };
      ...
      <div style={{maxWidth:480,margin:"0 auto",padding:"14px 14px 40px"}}>
        ...
        {tab==="log"&&logPhase==="active"&&<div>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:4}}>
            ...
            <button onClick={()=>setShowCancelConfirm(true)} style={{background:"none",border:"none",color:t.red,fontSize:10,fontWeight:600,cursor:"pointer"}}>Cancel</button>
          </div>
          ...
          {showCancelConfirm&&<div style={{position:"fixed",inset:0,background:t.overlay,display:"flex",alignItems:"center",justifyContent:"center",zIndex:100,padding:20}} onClick={()=>setShowCancelConfirm(false)}>
            <div style={{background:t.surface,borderRadius:16,border:`1px solid ${t.border}`,padding:20,maxWidth:300,width:"100%"}} onClick={e=>e.stopPropagation()}>
              <div style={{fontSize:16,fontWeight:700,color:t.text,marginBottom:4}}>Cancel Workout?</div>
              <div style={{fontSize:12,color:t.textSec,marginBottom:16,lineHeight:1.5}}>All logged data for this session will be lost.</div>
              <div style={{display:"flex",gap:8}}>
                <button onClick={()=>setShowCancelConfirm(false)} style={{flex:1,padding:11,borderRadius:10,border:`1px solid ${t.border}`,background:"transparent",color:t.textMuted,fontWeight:600,fontSize:13,cursor:"pointer"}}>Keep Going</button>
                <button onClick={()=>{setShowCancelConfirm(false);cancelWorkout();}} style={{flex:1,padding:11,borderRadius:10,border:"none",background:t.red,color:"#fff",fontWeight:700,fontSize:13,cursor:"pointer"}}>Discard</button>
              </div>
            </div>
          </div>}
            ...
            return <div style={{...cardWithTop(de.length?getSplitColor(de[0]?.split):CARDIO_COLOR),padding:"12px 14px",marginBottom:12}}>
              ...
              {isEditing?<div>
                {editingHistory.map((h,i)=><div key={i} style={{padding:"6px 0",borderTop:i>0?`1px solid ${t.borderLight}`:"none"}}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:4}}>
                    <button onClick={()=>{setHistorySwapIdx(historySwapIdx===i?null:i);setHistorySwapSearch("");}} style={{background:"none",border:"none",cursor:"pointer",padding:0,display:"flex",alignItems:"center",gap:3}}>
                      <span style={{fontSize:12,color:historySwapIdx===i?t.accent:t.textDim,fontWeight:600}}>{h.exercise}</span>
                      <span style={{fontSize:8,color:t.textFaint}}>✎</span>
                    </button>
                  ...
                  {historySwapIdx===i&&<div style={{marginBottom:6,padding:6,background:t.bg,borderRadius:8}}>
                    <input value={historySwapSearch} onChange={e=>setHistorySwapSearch(e.target.value)} placeholder="Search exercise..." autoFocus style={{width:"100%",padding:"6px 8px",borderRadius:6,border:`1px solid ${t.border}`,background:t.inputBg,color:t.text,fontSize:12,outline:"none",boxSizing:"border-box",marginBottom:4}}/>
                    {historySwapSearch.trim()&&<div style={{maxHeight:120,overflowY:"auto"}}>{EXERCISE_LIST.filter(e=>e.name.toLowerCase().includes(historySwapSearch.toLowerCase())).slice(0,8).map(e=><button key={e.name} onClick={()=>swapHistoryExercise(i,e)} style={{display:"block",width:"100%",textAlign:"left",padding:"5px 8px",background:"transparent",border:"none",cursor:"pointer",fontSize:11,color:t.text,borderRadius:4}}><span style={{fontWeight:600}}>{e.name}</span> <span style={{fontSize:9,color:MC[e.muscle]}}>{e.muscle}</span></button>)}</div>}
                  </div>}
              ...
              {dc.length>0&&<div style={{marginTop:de.length?8:0}}><div style={{fontSize:10,fontWeight:700,color:CARDIO_COLOR,marginBottom:3,textTransform:"uppercase"}}>🏃 Cardio</div>{dc.map((c,i)=><div key={i} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"4px 0",borderTop:i>0?`1px solid ${t.borderLight}`:"none"}}><span style={{fontSize:12,color:t.textDim}}>{c.type}</span><div style={{display:"flex",gap:8,fontSize:11,alignItems:"center"}}>{c.distance>0&&<span style={{color:CARDIO_COLOR,fontWeight:700}}>{c.distance}{c.unit}</span>}{c.time>0&&<span style={{color:t.textSec}}>{c.time}min</span>}{c.pace>0&&<span style={{color:t.textFaint,fontSize:9}}>{c.pace}min/{c.unit}</span>}<button onClick={()=>deleteCardioEntry(c)} style={{background:"none",border:"none",color:t.red,cursor:"pointer",fontSize:12,padding:"0 2px",marginLeft:2}}>×</button></div></div>)}</div>}
            </div>;
          })()}
        ...
        {tab==="progress"&&<div>
          ...
          <div style={{marginTop:20}}>
                ...
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                  ...
                  <div style={{display:"flex",alignItems:"center",gap:8,flex:1,minWidth:0}}>
                    ...
                    <div style={{minWidth:0,flex:1}}>
                      {editingDb&&renamingEx===ex.name?<div style={{display:"flex",gap:4,alignItems:"center"}}>
                        <input value={renameVal} onChange={e=>setRenameVal(e.target.value)} autoFocus onKeyDown={e=>{if(e.key==="Enter")renameExercise(ex.name,renameVal);if(e.key==="Escape"){setRenamingEx(null);setRenameVal("");}}} style={{flex:1,padding:"4px 6px",borderRadius:6,border:`1px solid ${t.border}`,background:t.inputBg,color:t.text,fontSize:13,outline:"none",fontWeight:600,boxSizing:"border-box",minWidth:0}}/>
                        <button onClick={()=>renameExercise(ex.name,renameVal)} style={{padding:"4px 8px",borderRadius:6,border:"none",background:t.green,color:"#fff",fontWeight:700,fontSize:10,cursor:"pointer",flexShrink:0}}>Save</button>
                        <button onClick={()=>{setRenamingEx(null);setRenameVal("");}} style={{padding:"4px 6px",borderRadius:6,border:`1px solid ${t.border}`,background:"transparent",color:t.textMuted,fontWeight:600,fontSize:10,cursor:"pointer",flexShrink:0}}>×</button>
                      </div>:<div style={{fontSize:13,fontWeight:600,color:t.text,display:"flex",alignItems:"center",gap:4}}>
                        {editingDb?<button onClick={()=>{setRenamingEx(ex.name);setRenameVal(ex.name);setEditingMuscleEx(null);setEditingRestEx(null);}} style={{background:"none",border:"none",cursor:"pointer",padding:0,display:"flex",alignItems:"center",gap:3}}><span style={{fontSize:13,fontWeight:600,color:t.text,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{ex.name}</span><span style={{fontSize:8,color:t.textFaint}}>✎</span></button>:<span style={{whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{ex.name}</span>}
                        {isCustom&&<span style={{fontSize:7,color:t.accent,fontWeight:700,background:t.accentBg,padding:"1px 4px",borderRadius:4,flexShrink:0}}>CUSTOM</span>}
                      </div>}
            ...
            <div style={{fontSize:9,color:t.textFaint,marginTop:6,textAlign:"center"}}>{EXERCISE_LIST.filter(e=>dbMuscleFilter==="All"||e.muscle===dbMuscleFilter).length} exercises{editingDb?" · Tap − to remove · Tap name to rename · Tap muscle to reassign":" · Tap rest time to edit"}</div>
          </div>
        </div>}
        ...
        {tab==="friends"&&<div>
          ...
          {(()=>{const f=friends[selFriend];if(!f)return null;
            ...
            <div style={{display:"flex",flexDirection:"column",gap:4,marginBottom:myFavsWithData.length>0&&friendOnlyExs.length>0?4:14}}>{myFavsWithData.map(ex=>{const fPR=getPR(f.history,ex),myPR=getPR(history,ex),ahead=myPR>fPR;return <div key={ex} onClick={()=>setChartEx(ex)} style={{...card,padding:"8px 12px",display:"flex",alignItems:"center",justifyContent:"space-between",cursor:"pointer",outline:chartEx===ex?`2px solid ${f.color}`:"none"}}><div style={{display:"flex",alignItems:"center",gap:4,flex:1,minWidth:0,overflow:"hidden"}}><span style={{fontSize:12,color:t.yellow}}>★</span><span style={{fontSize:11,fontWeight:500,color:chartEx===ex?f.color:t.textDim,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{ex}</span></div><div style={{display:"flex",alignItems:"center",gap:6,fontSize:11,flexShrink:0}}><span style={{color:f.color,fontWeight:700}}>{fPR||"—"}</span><span style={{color:t.textFaint,fontSize:8}}>vs</span><span style={{color:t.green,fontWeight:700}}>{myPR||"—"}</span>{myPR>0&&fPR>0&&<div style={{fontSize:8,fontWeight:700,padding:"1px 5px",borderRadius:5,background:ahead?t.greenBg:t.redBg,color:ahead?t.green:t.red}}>{ahead?"▲":"▼"}{Math.abs(myPR-fPR)}</div>}</div></div>})}</div>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:4,marginBottom:14}}>{friendOnlyExs.map(ex=>{const fPR=getPR(f.history,ex),myPR=getPR(history,ex),ahead=myPR>fPR;return <div key={ex} onClick={()=>setChartEx(ex)} style={{...card,padding:"8px 12px",display:"flex",alignItems:"center",justifyContent:"space-between",cursor:"pointer",outline:chartEx===ex?`2px solid ${f.color}`:"none"}}><div style={{fontSize:11,fontWeight:500,color:chartEx===ex?f.color:t.textDim,flex:1,minWidth:0,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{ex}</div><div style={{display:"flex",alignItems:"center",gap:6,fontSize:11,flexShrink:0}}><span style={{color:f.color,fontWeight:700}}>{fPR}</span><span style={{color:t.textFaint,fontSize:8}}>vs</span><span style={{color:t.green,fontWeight:700}}>{myPR||"—"}</span>{myPR>0&&<div style={{fontSize:8,fontWeight:700,padding:"1px 5px",borderRadius:5,background:ahead?t.greenBg:t.redBg,color:ahead?t.green:t.red}}>{ahead?"▲":"▼"}{Math.abs(myPR-fPR)}</div>}</div></div>})}</div></>}
